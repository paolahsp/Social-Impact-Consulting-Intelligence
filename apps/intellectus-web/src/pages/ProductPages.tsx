...
export function NewDiagnosticPage() {
  const { intake, prepareDiagnostic, runError, runStatus, runStatusMessage, supportingDocuments } = useDiagnostic()
  ...
  return <PageFrame>
    ...
    <form className="mt-9 space-y-7" noValidate onSubmit={submit}>
      ...
      <button className="primary-action" disabled={preparing} type="submit">Prepare diagnostic</button>{preparing && <p aria-live="polite" className="text-sm font-medium text-muted" role="status">{runStatusMessage}</p>}{!preparing && runStatus === 'failed' && <p className="text-sm font-medium text-danger" role="alert">{runError}</p>}
    </form>
  </PageFrame>
}
...
export function BriefPage() {
  ...
  return <PageFrame document>
    <article className="print-document min-w-0 border border-border bg-surface px-5 py-8 sm:px-9 sm:py-10 lg:px-14 lg:py-12">
      ...
      <details className="print-expand border-y border-border py-2"><summary className="flex min-h-12 cursor-pointer items-center font-semibold text-accent">Sources and limitations</summary><div className="space-y-4 pb-5 pt-4 text-sm leading-7 text-muted">{result!.diagnostic.isDemo ? <p>No reviewed public sources are included in this local demo result.</p> : <ul className="list-disc space-y-2 pl-5">{result!.diagnostic.sources.map((source) => <li key={source.id}>{source.url ? <a className="text-accent underline" href={source.url} rel="noreferrer" target="_blank">{source.title}</a> : source.title}</li>)}</ul>}<ul className="list-disc space-y-2 pl-5">{packageResult.confidence_limitations.map((item) => <li key={item}>{item}</li>)}</ul></div></details>
